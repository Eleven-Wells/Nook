  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#13171F] p-6 rounded-2xl border border-gray-200 dark:border-white/5 shadow-lg flex flex-col min-h-75 transition-colors">
        ...
        <div className="lg:col-span-3 bg-white dark:bg-[#13171F] p-6 rounded-2xl border border-gray-200 dark:border-white/5 shadow-lg flex flex-col transition-colors">
          ...
          <div className="flex-1 space-y-4">
            ...
            {paymentMethods.length === 0 && !isFormOpen && (
              <div className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed bg-gray-50 dark:bg-white/2 p-4 rounded-xl border border-gray-300 dark:border-gray-800 border-dashed">
            ...
            {!isFormOpen && paymentMethods.map((method) => (
              <div key={method.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-white/2 border border-gray-200 dark:border-white/5 rounded-xl group hover:border-green-500 dark:hover:border-green-700 transition-colors">
                ...
                <div className="flex flex-col items-end gap-2">
                  <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${method.status === 'Active' ? 'bg-[#1A8749]/10 text-[#22c55e]' : 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-500'
                    }`}>
                    {method.status}
                  </span>
            ...
            {/* Add / Edit Form */}
            {isFormOpen && (
              <form onSubmit={handleSavePayment} className="p-5 bg-gray-50 dark:bg-white/2 border border-[#22c55e]/30 rounded-xl space-y-4">
                <div>
          ...
          {!isFormOpen && paymentMethods.length < 3 && (
            <button
              ...
              className="mt-4 w-full py-4 border border-green-600 border-dashed rounded-xl text-sm font-semibold text-green-600 hover:text-green-700 dark:hover:text-white hover:border-green-700 dark:hover:border-green-800 hover:bg-green-50 dark:hover:bg-white/2 transition-all"
            >
              + Add Payout Method
            </button>
          )}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {transactions.map((tx, index) => (
                <tr key={index} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-white/2 transition-colors">
                  ...
                  <td className="py-4">
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-widest ${tx.type === 'Credit' ? 'text-[#22c55e] bg-[#1A8749]/10' : 'text-red-600 dark:text-red-500 bg-red-100 dark:bg-red-500/10'
                      }`}>
                      {tx.type}
                    </span>
                  </td>
                  <td className={`py-4 text-sm font-bold ${tx.amount.startsWith('+') ? 'text-[#22c55e]' : 'text-red-600 dark:text-red-500'
                    }`}>
                    {tx.amount}
                  </td>
                  <td className="py-4">
                    <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest border ${tx.status === 'Completed' ? 'border-[#22c55e]/30 text-[#22c55e]' : 'border-yellow-500/30 text-yellow-600 dark:text-yellow-500'
                      }`}>
                      {tx.status}
                    </span>
                  </td>
