    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {monetizationMetrics.map((metric, i) => (
        <div key={i} className="bg-[#13171F] p-6 rounded-2xl border border-white/10 shadow-lg">
          ...
          <div className={`text-3xl font-bold mb-2 ${metric.valueColor}`}>
  {metric.value}
</div>
...
<div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
  ...
  {/* Chart Area - Takes up 3/5 of the width */}
  <div className="lg:col-span-2 bg-[#13171F] p-6 rounded-2xl border border-white/5 shadow-lg flex flex-col min-h-[300px]">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={revenueData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
          ...
          <Line 
            ...
            stroke="#22c55e" 
            strokeWidth={3} 
            dot={{ r: 4, fill: '#13171F', stroke: '#22c55e', strokeWidth: 2 }} 
  ...
  {/* Payout Methods Area - Takes up 2/5 of the width */}
  <div className="lg:col-span-3 bg-[#13171F] p-6 rounded-2xl border border-white/5 shadow-lg flex flex-col">
    ...
    <div className="flex-1 space-y-4">
      ...
      {paymentMethods.length === 0 && !isFormOpen && (
        <div className="text-sm text-gray-400 leading-relaxed bg-white/[0.02] p-4 rounded-xl border border-gray-800 border-dashed">
          Add any payment method of your choice using the button below. <br/>
          <span className="text-yellow-500 mt-2 block text-xs">Note: You can only add up to 3 payment methods!</span>
        </div>
      )}

      {/* List of Saved Payment Methods */}
      {!isFormOpen && paymentMethods.map((method) => (
        <div key={method.id} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-xl group hover:border-green-700 transition-colors">
          <div className="flex items-center gap-4">
            <div className="bg-black p-3 rounded-2xl text-green-500">
              <CiBank size={18} />
            </div>
            <div>
              <h4 className="text-white text-sm font-medium">{method.type}</h4>
          ...
          <div className="flex flex-col items-end gap-2">
            <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${
              method.status === 'Active' ? 'bg-[#1A8749]/10 text-[#22c55e]' : 'bg-yellow-500/10 text-yellow-500'
            }`}>
              {method.status}
            </span>
            
            {/* Edit/Delete Actions (Appear on hover for clean UI) */}
            <div className="flex gap-3">
              <button onClick={() => handleEditClick(method)} className="text-gray-500 hover:text-[#22c55e] transition-colors">
                <FaEdit size={12} />
              </button>
              <button onClick={() => handleDeleteClick(method.id)} className="text-gray-500 hover:text-red-500 transition-colors">
                <FaTrash size={12} />
              </button>
            </div>
          </div>
      ...
      {/* Add / Edit Form */}
      {isFormOpen && (
        <form onSubmit={handleSavePayment} className="p-5 bg-white/[0.02] border border-[#22c55e]/30 rounded-xl space-y-4">
          <div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={handleCancelForm} className="flex-1 py-2.5 text-xs font-bold text-gray-400 hover:text-white bg-white/5 rounded-lg transition-colors">
              Cancel
            </button>
    ...
    {/* Add Button - Only shows if not currently adding/editing AND less than 3 methods */}
    {!isFormOpen && paymentMethods.length < 3 && (
      <button 
        onClick={() => setIsFormOpen(true)}
        className="mt-4 w-full py-4 border border-green-600 border-dashed rounded-xl text-sm font-semibold text-green-600 hover:text-white hover:border-green-800 hover:bg-white/[0.02] transition-all"
      >
        + Add Payout Method
      </button>
    )}
